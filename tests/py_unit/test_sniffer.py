...
def test_sniffer_pickup_simulation(engine):
    ...
    def mock_callback(packet_ptr):
        #  simulates the function that SnifferWrapper would call
        packet = packet_ptr.contents
        action = engine.evaluate(packet)
        results.append(action)

    # simulate a 'safe' Read Holding Register packet
    ...
    # simulate an 'alert' Critical Register Write packet
    ...
    # simulate an 'alert' Single Write packet (even if not critical)
